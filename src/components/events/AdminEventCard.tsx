    ...
    <>
      <div className="relative aspect-[16/9] w-full overflow-hidden bg-ink-5">
        ...
        <div className="absolute bottom-3 left-3 right-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-white/90">{event.category}</p>
      ...
      <div className={cn('space-y-3 p-4', !eligible && 'opacity-80')}>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          <MiniStat label="Tickets sold" value={event.ticketsSold.toLocaleString()} />
          <MiniStat
            label="Fill rate"
            value={`${fill}%`}
            valueClass={fill >= 88 ? 'text-mint' : fill >= 55 ? 'text-ink' : 'text-amber'}
          />
          <MiniStat label="Revenue" value={formatSarCompact(event.revenueSar)} valueClass="text-coral" />
          <MiniStat label="Avg rating" value={`${event.avgRating.toFixed(1)} ★`} valueClass="font-black text-ink" />
        </div>
        ...
        <div className="flex items-center justify-between gap-3 rounded-2xl border border-ink-10 bg-gradient-to-r from-mint/15 via-white to-lemon/20 px-3 py-2.5">
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-ink-40">Check-in success</p>
            <p className="text-[11px] font-semibold text-ink-60">Operational health vs. sold seats</p>
          </div>
          <p
            className={cn(
          ...
          >
            {event.status === 'cancelled' ? '—' : `${event.successRatePercent}%`}
          </p>
        </div>
